...
var _chatOpen=false,_pendingContext=[],_eventSource=null,_selectedText='',_selectedOrigin=null;
...
async function sendMessage(){if(!BASE){_appendMessage('assistant','Chat-Server nicht verfügbar.');return;}var input=document.getElementById('chat-input');var text=input.value.trim();if(!text)return;var context=_pendingContext.map(function(c){var pfx='';if(c.origin){var ps=[];if(c.origin.abschnitt)ps.push(c.origin.abschnitt);if(c.origin.kategorie)ps.push(c.origin.kategorie);if(c.origin.fehler_id)ps.push(c.origin.fehler_id);if(c.origin.fehlermodus)ps.push(c.origin.fehlermodus);if(ps.length)pfx='['+ps.join(' > ')+'] ';}return pfx+c.text;}).join(' | ');_appendMessage('user',text+(context?'\\n[Kontext: '+context+']':''));input.value='';_clearContextChips();var thinkId='thinking-'+Date.now();_appendMessage('assistant thinking','Warte auf Antwort…',thinkId);document.getElementById('send-btn').disabled=true;try{await fetch(BASE+'/chat/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:text,context:context})});}catch(e){_appendMessage('assistant','Fehler: '+e.message,thinkId);}finally{document.getElementById('send-btn').disabled=false;}}
...
function _onTextSelect(e){if(e&&e.target&&e.target.closest&&e.target.closest('#selection-popover'))return;var sel=window.getSelection();var txt=sel?sel.toString().trim():'';if(txt.length<10||!sel||sel.rangeCount===0){_hidePopover();return;}var rect=sel.getRangeAt(0).getBoundingClientRect();if(!rect||(rect.width===0&&rect.height===0)){_hidePopover();return;}_selectedText=txt;_selectedOrigin=_getSelectionOrigin();_showPopover(rect.left+rect.width/2,rect.top-52);}
function _showPopover(x,y){var pop=document.getElementById('selection-popover');var pw=_chatOpen?390:0;var maxX=window.innerWidth-pw-220;pop.style.display='block';pop.style.left=Math.max(10,Math.min(x,maxX))+'px';pop.style.top=Math.max(10,y)+'px';}
function _hidePopover(){document.getElementById('selection-popover').style.display='none';_selectedText='';_selectedOrigin=null;}
document.addEventListener('mousedown',function(e){if(!e.target.closest('#selection-popover'))_hidePopover();});
function _getSelectionOrigin(){var sel=window.getSelection();if(!sel||sel.rangeCount===0)return null;var node=sel.getRangeAt(0).startContainer;if(node.nodeType===3)node=node.parentElement;if(!node)return null;var o={};var card=node.closest('.risk-card');if(card){var idEl=card.querySelector('.card-id');var titleEl=card.querySelector('.card-title');if(idEl)o.fehler_id=idEl.textContent.trim();if(titleEl)o.fehlermodus=titleEl.textContent.trim();}var cat=node.closest('.category-block');if(cat){var ct=cat.querySelector('.category-name');if(ct)o.kategorie=ct.textContent.trim();}var sec=node.closest('.page-section');if(sec){var st=sec.querySelector('.section-title');if(st)o.abschnitt=st.textContent.trim();if(sec.id)o.section_id=sec.id;}return Object.keys(o).length>0?o:null;}
function addSelectionToContext(){if(!_selectedText)return;var id='ctx-'+Date.now();_pendingContext.push({id:id,text:_selectedText,origin:_selectedOrigin});_renderContextChips();_hidePopover();window.getSelection().removeAllRanges();if(!_chatOpen)toggleChat();}
function _renderContextChips(){var area=document.getElementById('context-chip-area');while(area.firstChild)area.removeChild(area.firstChild);_pendingContext.forEach(function(c){var chip=document.createElement('div');chip.className='context-chip';var label=document.createElement('span');var preview=c.text.length>40?c.text.slice(0,40)+'…':c.text;var tag='';if(c.origin){tag=c.origin.fehler_id||c.origin.kategorie||c.origin.abschnitt||'';}label.textContent=(tag?'['+tag+'] ':'')+'"'+preview+'"';label.title=(tag?tag+': ':'')+c.text;var btn=document.createElement('button');btn.textContent='✕';(function(id){btn.addEventListener('click',function(){_removeContextChip(id);});})(c.id);chip.appendChild(label);chip.appendChild(btn);area.appendChild(chip);});}
